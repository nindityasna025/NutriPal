      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 mx-auto">
        {featureCards.map(card => {
          const dialog = (
            <Dialog 
              open={
                (card.id === 'curation' && isDeliveryOpen) ||
                (card.id === 'predictive' && isMenuOpen) ||
            ...
            >
              <DialogTrigger asChild>
                <Card className="rounded-[2rem] border-none shadow-premium hover:shadow-premium-lg transition-all bg-white cursor-pointer group p-6 flex flex-col items-center justify-between text-center space-y-4 active:scale-[0.98]">
                  ...
                  <Button onClick={card.action} variant={card.color === 'primary' ? 'default' : 'secondary'} className={cn(
              ...
              {card.id === 'curation' && (
                <DialogContent className="max-w-4xl rounded-[2.5rem] p-0 border-none shadow-premium-lg bg-white w-[94vw] max-h-[90vh] flex flex-col">
                  ...
                  <DialogFooter className="p-6 border-t bg-background rounded-b-[2.5rem]">
                    <Button variant="outline" className="w-full" onClick={() => setIsDeliveryOpen(false)}>Close</Button>
                  </DialogFooter>
                </DialogContent>
              )}

              {card.id === 'predictive' && (
                <DialogContent className="max-w-6xl rounded-[2.5rem] p-0 border-none shadow-premium-lg bg-white w-[94vw] max-h-[90vh] flex flex-col">
                  <DialogHeader className="p-8 text-center border-b">
          ...
          return <div key={card.id}>{dialog}</div>
        })}
      </div>
