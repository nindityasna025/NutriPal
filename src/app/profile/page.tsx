    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 animate-in fade-in duration-500">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-8">
                <Card className="border-none shadow-premium rounded-[2rem] p-8 text-center flex flex-col items-center">
                    <div className="relative">
                        <Avatar className="w-32 h-32 border-4 border-white shadow-premium">
                          <AvatarImage src={user.photoURL || ""} />
                          <AvatarFallback className="bg-primary text-foreground text-4xl font-black uppercase">
                            {user.displayName?.charAt(0) || user.email?.charAt(0)}
                          </AvatarFallback>
                        </Avatar>
                        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                          <DialogTrigger asChild>
                            <Button variant="outline" className="absolute bottom-0 right-0 rounded-full h-10 w-10 p-0 border-white bg-primary text-foreground shadow-premium">
                              <Edit2 className="w-4 h-4" />
                            </Button>
                          </DialogTrigger>
                          <DialogContent className="max-w-xl rounded-[3rem] p-0 border-none bg-background w-[94vw] max-h-[90vh] flex flex-col overflow-hidden">
                    ...
                    <div className="space-y-2 mt-6">
                        <h2 className="text-3xl font-black tracking-tight uppercase leading-tight text-foreground">{user.displayName || "Demo User"}</h2>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-6 w-full">
                        <div className="bg-primary/5 rounded-2xl p-4">
                            ...
                            <p className="text-2xl font-black tracking-tight uppercase text-foreground">{profile?.weight || "--"} <span className="text-xs font-black text-foreground opacity-20">kg</span></p>
                        </div>
                         <div className="bg-blue-50 rounded-2xl p-4">
                            <p className="text-[8px] font-black text-foreground opacity-40 uppercase tracking-widest">Status</p>
                            <p className="text-2xl font-black tracking-tight uppercase text-foreground">{profile?.bmiCategory || "Healthy"}</p>
                        </div>
                    </div>
                </Card>
            ...
            {/* Right Column: Health and Settings */}
            <div className="lg:col-span-8 space-y-8">
              <Card className="border-none shadow-premium bg-white rounded-[2.5rem] p-8">
                  ...
                  <div className="space-y-3 pt-6 border-t border-muted/30">
                      <p className="text-[10px] font-black uppercase tracking-widest text-foreground flex items-center gap-2">
